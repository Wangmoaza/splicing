...
high_group = df[(df['HRDscore'] > df['HRDscore'].median()) & (df['Sig3'] > df['Sig3'].median())].index
low_group = df[(df['HRDscore'] < df['HRDscore'].median()) & (df['Sig3'] < df['Sig3'].median())].index
new_ser = pd.Series(index=list(high_group) + list(low_group))
new_ser[high_group] = 'high'
new_ser[low_group] = 'low'
new_ser.to_csv('../Analysis/HR_leafviz/HRD_sig3_median_groups.txt', sep='\t', header=False)

# median is based on all samples not just BRCA free samples
high_group = free_df[(free_df['HRDscore'] > df['HRDscore'].median()) & (free_df['Sig3'] > df['Sig3'].median())].index
low_group = free_df[(free_df['HRDscore'] < df['HRDscore'].median()) & (free_df['Sig3'] < df['Sig3'].median())].index
new_ser = pd.Series(index=list(high_group) + list(low_group))
new_ser[high_group] = 'high'
new_ser[low_group] = 'low'
new_ser.to_csv('../Analysis/HR_leafviz/BRCA_free.HRD_sig3_median_groups.txt', sep='\t', header=False)
